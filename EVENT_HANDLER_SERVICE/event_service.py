...
PROMETHEUS_DATA = {
    ...
    "remove_rating_counter": Counter(
        "remove_rating_counter", "Number of remove rating calls"
    ),
}
...
def start_event(target, event):
    print(event)
    print(event["message"].get("username", None) is None)
    sys.stdout.flush()
...
def delta(event, connection):
    ...
    day = current_day()
    cursor.execute(
        "SELECT * FROM basechel WHERE peerid=%s and id=%s", [int(string), int(id)]
    )
    ...
    if chel != None:
        ...
        cursor.execute(
            "UPDATE basechel set len = %s, time = %s where globalid = %s",
    ...
    if chel == None:
        ...
        if not event["message"].get("username", None) is None:
            new_chel = gen_new_tg(id)
            new_chel["name"] = event["message"].get("username")
        else:
            new_chel = gen_new(id)
        if new_chel == 0:
            returnValue = [
            ...
            cursor.execute(
                "INSERT INTO basechel (peerid, id, len, time, name, delta) VALUES(%s, %s, %s, %s, %s, %s)",
                tchel,
            )
            connection.commit()
    cursor.close()
...
def rand_gachi_text():
    gachi = [
        "Гей, приятель, я думаю, ти помилився дверима, Шкіряний клуб знаходиться в двох кварталах звідси."
        + "\n ",
        "трахнути тебе♂" + "\n ",
...
def remove_rating(event, connection):
    ...
    if chel == None:
        ...
        cursor.execute(
            "DELETE FROM basechel WHERE peerid=%s and id=%s", (int(string), int(id))
        )
        returnValue = [
            (
                "bot",
            ...
            while True:
                ...
                if event[-2] == "tg":
                    ...
                    channel.basic_publish(
                        exchange="", routing_key="sendQueueTG", body=json.dumps(event)
                    )
                else:
                    print("innerQueueManager", event)
                    sys.stdout.flush()
                    channel.basic_publish(
                        exchange="", routing_key="sendQueue", body=json.dumps(event)
                    )
...
if __name__ == "__main__":
    ...
    apiServiceURL = (
        "http://vk_api_send_dev:7331/api"
        if isProdigy
        else "http://vk_api_send:7331/api"
    )
    ...
    innerQueueProcess = multiprocessing.Process(
        target=innerQueueManager,
        args=(
            innerQueue,
            isProdigy,
        ),
    ...
    channel.basic_consume(
        queue="eventQueue", auto_ack=True, on_message_callback=callback
    )
